...
func Bootstrap(cfg *BootstrapConfig) {
	...
	// Use Case Config
	waClient := pkg.NewWapiSenderClient(cfg.Config, cfg.Log)
	...
	propertyUseCase := usecase.NewPropertyUseCase(cfg.DB, cfg.Log, cfg.Validate, propertyRepo, propertyImageRepo, hostProfileRepo, waClient, cfg.Mail, cfg.Config.GetString("app.cors_origins"))
	...
	bookingUseCase := usecase.NewBookingUseCase(cfg.DB, cfg.Log, cfg.Validate, bookingRepo, rentableRepo, availabilityRepo, propertyRepo, hostProfileRepo, userRepo, waClient, cfg.Mail)
	...
	paymentUseCase := usecase.NewPaymentUseCase(cfg.DB, cfg.Log, bookingRepo, userRepo, paymentRepo, propertyRepo, dokuClient, waClient, cfg.Mail)
