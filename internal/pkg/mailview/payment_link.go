...
// PaymentLinkMailView renders an email with the payment URL for the guest.
func PaymentLinkMailView(name, bookingID, paymentURL string, totalPrice float64) string {
	return fmt.Sprintf(`
		<!DOCTYPE html>
		<html>
						...
						<table width="600" style="background:#ffffff; padding:40px; border-radius:16px; border:1px solid #e5e5e5;">
							...
							<tr>
								<td style="padding-top:16px; font-size:16px; color:#444; line-height:1.6;">
									Halo <b>%s</b>, link pembayaran untuk booking Anda sudah siap. Segera selesaikan sebelum batas waktu habis.
								</td>
							</tr>
	...
	`, name, bookingID, totalPrice, paymentURL, paymentURL, paymentURL)
}
